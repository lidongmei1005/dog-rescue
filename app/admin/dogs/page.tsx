...
interface Dog {
  id: number; name: string; breed: string; age: string; status: string; gender: string;
}
...
async function updateStatus(formData: FormData) {
  ...
  const db = getDb();
  db.prepare("UPDATE dogs SET status = ? WHERE id = ?").run(status, id);
  revalidatePath("/admin/dogs");
  revalidatePath("/dogs");
}
...
export default function AdminDogsPage() {
  const db = getDb();
  const dogs = db.prepare("SELECT * FROM dogs ORDER BY created_at DESC").all() as Dog[];

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-amber-900">Manage Dogs</h1>
        <Link href="/admin/dogs/new" className="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2 rounded-full font-semibold">
          + Add Dog
        </Link>
      </div>
            ...
            {dogs.map((dog) => (
              <tr key={dog.id} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <StatusSelect id={dog.id} status={dog.status} action={updateStatus} />
                </td>
                <td className="px-4 py-3 flex gap-3">
                  ...
                  <form action={deleteDog} className="inline">
                    ...
                    <button type="submit" className="text-red-500 hover:underline" formAction={deleteDog}>
                      Delete
                    </button>
                  </form>
                </td>
