...
import AppStatusSelect from "@/app/admin/applications/AppStatusSelect";

interface Application {
  id: number; dog_name: string; applicant_name: string; email: string;
  phone: string; reason: string; home_type: string; other_pets: string;
  status: string; created_at: string;
}

async function updateAppStatus(formData: FormData) {
  "use server";
  await execute("UPDATE adoption_applications SET status = ? WHERE id = ?",
...
export default async function ShelterApplicationsPage() {
  ...
  const apps = await query<Application>(
    `SELECT a.* FROM adoption_applications a JOIN dogs d ON a.dog_id = d.id WHERE d.shelter_id = ? ORDER BY a.created_at DESC`,
    [user!.id]
  );
